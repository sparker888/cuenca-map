// UI string dictionary (EN / ES) for the chrome — nav, headers, footers, map UI.
// Long-form review article bodies are not translated here; route-based i18n
// (/es/…) is the recommended production approach for full content translation.

export type Lang = "en" | "es";

export const UI: Record<Lang, Record<string, string>> = {
  en: {
    nav_map: "Map", nav_reviews: "Reviews", nav_design: "Design system", nav_about: "About", nav_list: "List your business",
    hero_eyebrow: "Ecuador - Andes Mountains",
    hero_title: "The places expats love in Cuenca",
    hero_dek: "An honest, hand-mapped guide to El Centro and beyond — landmarks, markets, parks, and the cafés worth a long morning.",
    chip_places_word: "places", chip_cats_word: "categories", chip_reviews: "Read the reviews →",
    map_caption: "Tap a gold pin to open the listing — featured spots link straight to our full review.",
    filter_all: "All",
    cat_landmark: "Landmarks & plazas", cat_market: "Markets", cat_museum: "Museums & culture", cat_outdoors: "Parks & views", cat_food: "Food & cafés",
    "cat_restaurants-cafes": "Restaurants & cafés", cat_dental: "Dental", "cat_real-estate": "Real estate", "cat_visa-legal": "Visa & legal", cat_tours: "Tours", cat_bars: "Bars",
    legend_reviewed: "Reviewed",
    panel_hours: "Hours", panel_directions: "Get directions", panel_website: "Website",
    panel_review: "Read our full review", panel_claim: "Own this business? Get a featured listing →",
    panel_specials: "Current specials",
    fav: "Local favorite",
    show_eyebrow: "From the guide", show_title: "Restaurants we've reviewed",
    show_dek: "We eat everywhere so you don't have to. These are the spots we keep going back to — written up in full, no sponsored fluff.",
    all_reviews: "All reviews", card_read: "Read review", card_soon: "Review coming soon", badge_soon: "Coming soon",
    foot_tagline: "A small, independent guide to living and eating well in Cuenca, Ecuador. Made by people who actually live here.",
    foot_explore: "Explore", foot_map: "The map", foot_own: "Own a business?",
    foot_own_body: "Get a featured listing and a full review.", foot_own_link: "Get in touch →",
    foot_legal: "© 2026 Cuenca Expat Map · Map data © OpenStreetMap, CARTO",
    rev_eyebrow: "Independent & unsponsored", rev_title: "Reviews",
    rev_dek: "We write up the places we actually go back to — in full, in our own words, with no one paying for the privilege. Start here.",
    rev_latest: "Latest reviews", editors_pick: "★ Editors' pick", read_full: "Read the full review",
    bc_back: "← All reviews", keep_reading: "Keep reading", more_guide: "More from the guide",
    about_eyebrow: "About · the maker", about_title: "Remote Solopreneur",
    about_bio: "I'm a one-person business run from a sunny apartment in Cuenca — and I make videos about the move abroad: visas, the real cost of living, and the slow, good day-to-day of expat life in the Ecuadorian Andes. This guide is the map I wish I'd had when I landed.",
    about_sub: "Subscribe on YouTube", about_news: "Get the newsletter",
    watch_eyebrow: "Watch", videos_title: "Videos & guides", new_badge: "New · In production",
    biz_eyebrow: "For business owners", cta_title: "Is your business missing?", cta_title2: "Add it to the map.",
    cta_body: "A pin on the map is always free. Featured listings — photos and a full, honest review — are how the service keeps the lights on, but there's no hard sell here. Tell us about your place and we'll take it from there.",
    form_name: "Your name", form_business: "Business name", form_email: "Email", form_message: "Anything we should know? (optional)", form_send: "Send", form_thanks: "Thanks — we'll be in touch soon.", form_error: "Something went wrong — please try again.",
    list_eyebrow: "For business owners", list_title: "List your business in Cuenca",
    list_dek: "A pin on the map is always free. Featured listings — photos, hours, live specials, and a full honest review — are limited per category. Here's what's open right now.",
    list_avail: "Availability by category", list_remaining: "open", list_full: "Full — join the waitlist",
    wl_title: "Join the waitlist", wl_dek: "Tell us about your place and which category you're in. We'll reach out as a featured slot opens up.",
    wl_cat: "Category", wl_send: "Join the waitlist", wl_thanks: "You're on the list — we'll be in touch soon.",
    // Owner portal (/owner)
    o_eyebrow: "For business owners", o_title: "My Business Settings",
    o_signin: "Sign in", o_signin_sub: "Manage your listing, specials, and photos.",
    o_email: "Email", o_password: "Password", o_or: "or", o_google: "Sign in with Google",
    o_signed_as: "Signed in as", o_signout: "Sign out", o_loading: "Loading…",
    o_no_listing_title: "No listing assigned yet",
    o_no_listing_body: "Your account isn't linked to a business. Once we assign your listing you'll be able to manage it here.",
    o_free_msg: "Your listing is on the Free tier — a pin on the map. Upgrade to Enhanced to edit your tagline, hours, photos and post specials, or Premium for a full review and menu.",
    o_tagline: "Tagline", o_description: "Description", o_hours: "Hours (one line per row)",
    o_phone: "Phone", o_whatsapp: "WhatsApp", o_website: "Website", o_price: "Price",
    o_save: "Save changes", o_saved: "Saved ✓", o_save_failed: "Save failed",
    o_photos: "Photos", o_gallery: "Photo gallery", o_upto: "up to", o_add_photo: "+ Add photo",
    o_no_photos: "No photos yet.", o_limit_reached: "Limit reached", o_uploading: "Uploading…",
    o_del_photo: "Delete this photo?", o_upload_failed: "Upload failed",
    o_menu: "Menu (PDF)", o_menu_view: "View current menu (PDF)", o_menu_none: "No menu uploaded.", o_menu_upload: "Upload menu PDF",
    o_specials: "Specials & events", o_new: "+ New",
    o_specials_hint: "Active specials appear live on your map listing instantly — no rebuild.",
    o_no_specials: "No specials yet.", o_live: "live", o_off: "off",
    o_activate: "Activate", o_deactivate: "Deactivate", o_edit: "Edit", o_delete: "Delete",
    o_del_special: "Delete this special?", o_prompt_title: "Special title:", o_prompt_desc: "Description (optional):",
    o_subscription: "Subscription", o_sub_tier: "Tier", o_sub_status: "status", o_sub_paid_through: "paid through",
    o_sub_none_note: "no active subscription record.", o_sub_readonly: "(Read-only — managed by Cuenca Maps.)",
    o_sub_unavailable: "Subscription unavailable.",
    o_signin_failed: "Sign in failed.", o_google_unavailable: "Google sign-in is not available yet. Use email & password for now.",
    // WhatsApp + directions (task 03)
    wa_message: "Message on WhatsApp", wa_share: "Share", wa_claim: "Claim on WhatsApp",
    dir_get: "Get directions", dir_apple: "Apple Maps", dir_google: "Google Maps",
    wa_msg_text: "Hi! I found you on Cuenca Maps and wanted to get in touch.",
    wa_claim_text: "Hi, I'd like to list/claim my business on Cuenca Maps.",
    wa_share_text: "Check out {name} on Cuenca Maps: {url}",
    // Ops dashboard (task 15). Tier names (Free/Enhanced/Premium), DeUna, Payphone,
    // USD are product/proper nouns and intentionally kept identical across locales.
    d_eyebrow: "Cuenca Maps internal", d_title: "Operations",
    d_signin: "Sign in", d_signin_sub: "Sales & admin only.",
    d_email: "Email", d_password: "Password", d_google: "Sign in with Google",
    d_signin_failed: "Sign in failed.", d_google_unavailable: "Google sign-in unavailable.",
    d_signed_as: "Signed in as", d_signout: "Sign out", d_loading: "Loading…",
    d_tab_pipeline: "Pipeline", d_tab_sale: "Record a sale", d_tab_listings: "Listings", d_tab_leads: "Leads",
    d_stat_open_slots: "Open paid slots", d_stat_review_wait: "Premium awaiting a review",
    d_stat_renewals: "Renewals within 30 days", d_stat_leads: "Leads in the inbox",
    d_inventory: "Inventory — open slots", d_inventory_sub: "Paid listings vs cap. Scarcity is the product.",
    d_of: "of", d_open: "open",
    d_renewals: "Renewals approaching", d_renewals_sub: "Subscriptions paid through the next 30 days.",
    d_nothing_due: "Nothing due soon.",
    d_review_queue: "Review queue", d_review_queue_sub: "Premium listings with no published review yet.",
    d_all_reviewed: "All premium listings are reviewed. 🎉",
    d_sale_sub: "Atomic: it creates/links the owner account, sets the tier, and writes the subscription in one step.",
    d_mode_existing: "Upgrade existing listing", d_mode_new: "New business",
    d_free_to_upgrade: "Free listing to upgrade", d_select_free: "— select a free listing —",
    d_only_free_hint: "Only free pins in sellable categories can be upgraded. A paid listing is rejected by the server.",
    d_biz_name: "Business name", d_category: "Category", d_address_opt: "Address (optional)",
    d_location_hint: "Location — click the map to drop the pin", d_picked: "Picked:", d_none: "none",
    d_tier: "Tier", d_period: "Period", d_method: "Method",
    d_period_annual: "Annual", d_period_quarterly: "Quarterly", d_period_monthly: "Monthly",
    d_method_transfer: "Transfer", d_method_cash: "Cash", d_method_other: "Other",
    d_amount: "Amount", d_currency: "Currency", d_paid_through: "Paid through",
    d_owner_email: "Owner email", d_owner_name_opt: "Owner name (optional)",
    d_record_sale: "Record sale", d_recording: "Recording…",
    d_pick_listing: "Pick a listing to upgrade.", d_enter_name: "Enter a business name.",
    d_click_map: "Click the map to set the location.", d_owner_required: "Owner email is required.",
    d_sale_recorded: "Sale recorded ✓", d_is_now: "is now", d_and_live: "and live on the map",
    d_subscription_label: "Subscription", d_send_login: "Send the owner their login",
    d_email_label: "Email:", d_temp_password: "Temporary password:",
    d_shown_once: "Shown once. Send it via WhatsApp/email; they can also sign in with Google using this email.",
    d_linked_existing_a: "Linked to existing owner", d_linked_existing_b: "— no new login needed.",
    d_reflects: "The public site reflects this within ~1 minute (no manual deploy).",
    d_sale_failed: "Failed to record sale.",
    d_search: "Search", d_search_ph: "name or address…", d_all: "All",
    d_published: "Published", d_yes: "Yes", d_no: "No",
    d_col_name: "Name", d_col_pub: "Pub", d_col_claimed: "Claimed", d_col_review: "Review",
    d_listings_word: "listings", d_claimed_yes: "✓ claimed", d_review_yes: "★ yes", d_no_match: "No matching listings.",
    d_leads_inbox: "Leads inbox", d_lead_sing: "lead", d_lead_plur: "leads", d_newest_first: "Newest first.",
    d_interested_in: "Interested in", d_via: "via", d_convert: "Convert to sale →", d_no_leads: "No leads yet.",
  },
  es: {
    nav_map: "Mapa", nav_reviews: "Reseñas", nav_design: "Sistema de diseño", nav_about: "Acerca de", nav_list: "Publica tu negocio",
    hero_eyebrow: "Ecuador - Cordillera de los Andes",
    hero_title: "Los lugares que enamoran a los expats en Cuenca",
    hero_dek: "Una guía honesta y trazada a mano de El Centro y sus alrededores: monumentos, mercados, parques y los cafés que merecen una mañana entera.",
    chip_places_word: "lugares", chip_cats_word: "categorías", chip_reviews: "Lee las reseñas →",
    map_caption: "Toca un pin dorado para abrir la ficha: los lugares destacados enlazan directo a nuestra reseña completa.",
    filter_all: "Todos",
    cat_landmark: "Monumentos y plazas", cat_market: "Mercados", cat_museum: "Museos y cultura", cat_outdoors: "Parques y miradores", cat_food: "Comida y cafés",
    "cat_restaurants-cafes": "Restaurantes y cafés", cat_dental: "Dental", "cat_real-estate": "Bienes raíces", "cat_visa-legal": "Visa y legal", cat_tours: "Tours", cat_bars: "Bares",
    legend_reviewed: "Reseñado",
    panel_hours: "Horario", panel_directions: "Cómo llegar", panel_website: "Sitio web",
    panel_review: "Leer nuestra reseña completa", panel_claim: "¿Es tu negocio? Consigue una ficha destacada →",
    panel_specials: "Especiales actuales",
    fav: "Favorito local",
    show_eyebrow: "De la guía", show_title: "Restaurantes que hemos reseñado",
    show_dek: "Comemos en todas partes para que tú no tengas que hacerlo. Estos son los lugares a los que volvemos, reseñados a fondo y sin relleno patrocinado.",
    all_reviews: "Todas las reseñas", card_read: "Leer reseña", card_soon: "Reseña próximamente", badge_soon: "Próximamente",
    foot_tagline: "Una pequeña guía independiente para vivir y comer bien en Cuenca, Ecuador. Hecha por gente que vive aquí.",
    foot_explore: "Explorar", foot_map: "El mapa", foot_own: "¿Tienes un negocio?",
    foot_own_body: "Consigue una ficha destacada y una reseña completa.", foot_own_link: "Escríbenos →",
    foot_legal: "© 2026 Cuenca Expat Map · Datos del mapa © OpenStreetMap, CARTO",
    rev_eyebrow: "Independiente y sin patrocinio", rev_title: "Reseñas",
    rev_dek: "Escribimos sobre los lugares a los que de verdad volvemos: a fondo, con nuestras palabras y sin que nadie pague por aparecer. Empieza aquí.",
    rev_latest: "Últimas reseñas", editors_pick: "★ Selección del editor", read_full: "Leer la reseña completa",
    bc_back: "← Todas las reseñas", keep_reading: "Seguir leyendo", more_guide: "Más de la guía",
    about_eyebrow: "Acerca de · el autor", about_title: "Remote Solopreneur",
    about_bio: "Soy un negocio de una sola persona gestionado desde un departamento soleado en Cuenca, y hago videos sobre mudarse al extranjero: visas, el costo de vida real y el día a día tranquilo de la vida expat en los Andes ecuatorianos. Esta guía es el mapa que ojalá hubiera tenido al llegar.",
    about_sub: "Suscríbete en YouTube", about_news: "Recibe el boletín",
    watch_eyebrow: "Ver", videos_title: "Videos y guías", new_badge: "Nuevo · En producción",
    biz_eyebrow: "Para negocios", cta_title: "¿Falta tu negocio?", cta_title2: "Súmalo al mapa.",
    cta_body: "Un pin en el mapa siempre es gratis. Las fichas destacadas — fotos y una reseña completa y honesta — son lo que mantiene vivo el servicio, pero aquí no hay presión. Cuéntanos sobre tu lugar y nos encargamos del resto.",
    form_name: "Tu nombre", form_business: "Nombre del negocio", form_email: "Correo", form_message: "¿Algo que debamos saber? (opcional)", form_send: "Enviar", form_thanks: "Gracias, te escribiremos pronto.", form_error: "Algo salió mal — inténtalo de nuevo.",
    list_eyebrow: "Para negocios", list_title: "Publica tu negocio en Cuenca",
    list_dek: "Un pin en el mapa siempre es gratis. Las fichas destacadas — fotos, horario, especiales en vivo y una reseña completa y honesta — son limitadas por categoría. Esto es lo que hay disponible ahora.",
    list_avail: "Disponibilidad por categoría", list_remaining: "disponibles", list_full: "Lleno — únete a la lista",
    wl_title: "Únete a la lista de espera", wl_dek: "Cuéntanos sobre tu lugar y en qué categoría estás. Te contactaremos cuando se abra un cupo destacado.",
    wl_cat: "Categoría", wl_send: "Unirme a la lista", wl_thanks: "Estás en la lista — te escribiremos pronto.",
    // Portal del propietario (/owner)
    o_eyebrow: "Para negocios", o_title: "Configuración de mi negocio",
    o_signin: "Iniciar sesión", o_signin_sub: "Administra tu ficha, especiales y fotos.",
    o_email: "Correo", o_password: "Contraseña", o_or: "o", o_google: "Iniciar sesión con Google",
    o_signed_as: "Sesión iniciada como", o_signout: "Cerrar sesión", o_loading: "Cargando…",
    o_no_listing_title: "Aún no tienes una ficha asignada",
    o_no_listing_body: "Tu cuenta no está vinculada a un negocio. Cuando asignemos tu ficha podrás administrarla aquí.",
    o_free_msg: "Tu ficha está en el plan Gratis — un pin en el mapa. Pasa a Enhanced para editar tu eslogan, horario y fotos y publicar especiales, o a Premium para una reseña completa y menú.",
    o_tagline: "Eslogan", o_description: "Descripción", o_hours: "Horario (una línea por fila)",
    o_phone: "Teléfono", o_whatsapp: "WhatsApp", o_website: "Sitio web", o_price: "Precio",
    o_save: "Guardar cambios", o_saved: "Guardado ✓", o_save_failed: "Error al guardar",
    o_photos: "Fotos", o_gallery: "Galería de fotos", o_upto: "hasta", o_add_photo: "+ Agregar foto",
    o_no_photos: "Aún no hay fotos.", o_limit_reached: "Límite alcanzado", o_uploading: "Subiendo…",
    o_del_photo: "¿Eliminar esta foto?", o_upload_failed: "Error al subir",
    o_menu: "Menú (PDF)", o_menu_view: "Ver menú actual (PDF)", o_menu_none: "No hay menú subido.", o_menu_upload: "Subir menú PDF",
    o_specials: "Especiales y eventos", o_new: "+ Nuevo",
    o_specials_hint: "Los especiales activos aparecen al instante en tu ficha del mapa — sin reconstrucción.",
    o_no_specials: "Aún no hay especiales.", o_live: "activo", o_off: "inactivo",
    o_activate: "Activar", o_deactivate: "Desactivar", o_edit: "Editar", o_delete: "Eliminar",
    o_del_special: "¿Eliminar este especial?", o_prompt_title: "Título del especial:", o_prompt_desc: "Descripción (opcional):",
    o_subscription: "Suscripción", o_sub_tier: "Plan", o_sub_status: "estado", o_sub_paid_through: "pagado hasta",
    o_sub_none_note: "sin registro de suscripción activa.", o_sub_readonly: "(Solo lectura — gestionado por Cuenca Maps.)",
    o_sub_unavailable: "Suscripción no disponible.",
    o_signin_failed: "Error al iniciar sesión.", o_google_unavailable: "El inicio con Google aún no está disponible. Usa correo y contraseña por ahora.",
    // WhatsApp + cómo llegar (tarea 03)
    wa_message: "Mensaje por WhatsApp", wa_share: "Compartir", wa_claim: "Reclamar por WhatsApp",
    dir_get: "Cómo llegar", dir_apple: "Apple Maps", dir_google: "Google Maps",
    wa_msg_text: "¡Hola! Los encontré en Cuenca Maps y quería contactarlos.",
    wa_claim_text: "Hola, me gustaría listar/reclamar mi negocio en Cuenca Maps.",
    wa_share_text: "Mira {name} en Cuenca Maps: {url}",
    // Panel de operaciones (tarea 15). Los nombres de plan (Free/Enhanced/Premium),
    // DeUna, Payphone y USD son nombres propios y se mantienen igual en ambos idiomas.
    d_eyebrow: "Interno de Cuenca Maps", d_title: "Operaciones",
    d_signin: "Iniciar sesión", d_signin_sub: "Solo ventas y administración.",
    d_email: "Correo", d_password: "Contraseña", d_google: "Iniciar sesión con Google",
    d_signin_failed: "Error al iniciar sesión.", d_google_unavailable: "Inicio de sesión con Google no disponible.",
    d_signed_as: "Sesión iniciada como", d_signout: "Cerrar sesión", d_loading: "Cargando…",
    d_tab_pipeline: "Embudo", d_tab_sale: "Registrar venta", d_tab_listings: "Fichas", d_tab_leads: "Prospectos",
    d_stat_open_slots: "Cupos pagados disponibles", d_stat_review_wait: "Premium en espera de reseña",
    d_stat_renewals: "Renovaciones en 30 días", d_stat_leads: "Prospectos en la bandeja",
    d_inventory: "Inventario — cupos disponibles", d_inventory_sub: "Fichas pagadas vs. límite. La escasez es el producto.",
    d_of: "de", d_open: "disponibles",
    d_renewals: "Renovaciones próximas", d_renewals_sub: "Suscripciones pagadas hasta los próximos 30 días.",
    d_nothing_due: "Nada próximo a vencer.",
    d_review_queue: "Cola de reseñas", d_review_queue_sub: "Fichas premium sin reseña publicada aún.",
    d_all_reviewed: "Todas las fichas premium están reseñadas. 🎉",
    d_sale_sub: "Atómico: crea/vincula la cuenta del propietario, asigna el plan y registra la suscripción en un solo paso.",
    d_mode_existing: "Mejorar ficha existente", d_mode_new: "Negocio nuevo",
    d_free_to_upgrade: "Ficha gratis para mejorar", d_select_free: "— elige una ficha gratis —",
    d_only_free_hint: "Solo se pueden mejorar pines gratis en categorías vendibles. El servidor rechaza una ficha ya pagada.",
    d_biz_name: "Nombre del negocio", d_category: "Categoría", d_address_opt: "Dirección (opcional)",
    d_location_hint: "Ubicación — haz clic en el mapa para colocar el pin", d_picked: "Elegido:", d_none: "ninguno",
    d_tier: "Plan", d_period: "Periodo", d_method: "Método",
    d_period_annual: "Anual", d_period_quarterly: "Trimestral", d_period_monthly: "Mensual",
    d_method_transfer: "Transferencia", d_method_cash: "Efectivo", d_method_other: "Otro",
    d_amount: "Monto", d_currency: "Moneda", d_paid_through: "Pagado hasta",
    d_owner_email: "Correo del propietario", d_owner_name_opt: "Nombre del propietario (opcional)",
    d_record_sale: "Registrar venta", d_recording: "Registrando…",
    d_pick_listing: "Elige una ficha para mejorar.", d_enter_name: "Ingresa un nombre de negocio.",
    d_click_map: "Haz clic en el mapa para fijar la ubicación.", d_owner_required: "El correo del propietario es obligatorio.",
    d_sale_recorded: "Venta registrada ✓", d_is_now: "ahora es", d_and_live: "y está activo en el mapa",
    d_subscription_label: "Suscripción", d_send_login: "Envía al propietario su acceso",
    d_email_label: "Correo:", d_temp_password: "Contraseña temporal:",
    d_shown_once: "Se muestra una sola vez. Envíala por WhatsApp/correo; también pueden iniciar sesión con Google usando este correo.",
    d_linked_existing_a: "Vinculado al propietario existente", d_linked_existing_b: "— no se necesita un nuevo acceso.",
    d_reflects: "El sitio público refleja esto en ~1 minuto (sin despliegue manual).",
    d_sale_failed: "No se pudo registrar la venta.",
    d_search: "Buscar", d_search_ph: "nombre o dirección…", d_all: "Todas",
    d_published: "Publicado", d_yes: "Sí", d_no: "No",
    d_col_name: "Nombre", d_col_pub: "Pub", d_col_claimed: "Reclamado", d_col_review: "Reseña",
    d_listings_word: "fichas", d_claimed_yes: "✓ reclamado", d_review_yes: "★ sí", d_no_match: "No hay fichas que coincidan.",
    d_leads_inbox: "Bandeja de prospectos", d_lead_sing: "prospecto", d_lead_plur: "prospectos", d_newest_first: "Más recientes primero.",
    d_interested_in: "Interesado en", d_via: "vía", d_convert: "Convertir en venta →", d_no_leads: "Aún no hay prospectos.",
  },
};

// English review-card category label → i18n key
export const CAT_KEY_BY_LABEL: Record<string, string> = {
  "Landmarks & plazas": "cat_landmark", "Markets": "cat_market", "Museums & culture": "cat_museum", "Parks & views": "cat_outdoors", "Food & cafés": "cat_food", "Restaurants & cafés": "cat_restaurants-cafes",
};

export function getLang(): Lang {
  if (typeof localStorage === "undefined") return "en";
  return (localStorage.getItem("cuenca-lang") as Lang) || "en";
}
export function t(key: string, lang: Lang = getLang()): string {
  return UI[lang]?.[key] ?? UI.en[key] ?? key;
}
